...
int dl_file_input(int sensor_id, float temp, float rel_hum, int flags) {
  ...
  time_t cur_time;
  struct tm ts;
  if ((time( &cur_time ) == 0) || (gmtime_r(&cur_time, &ts) == 0)) {
  ...
  logging_info( "%04i-%02i-%02i %02i:%02i:%02i, %lli, %i, %1.2f, %1.2f, %i.\n", ts.tm_year+1900, ts.tm_mon+1, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec, (long long int)time_sec, sensor_id, temp, rel_hum, flags );
